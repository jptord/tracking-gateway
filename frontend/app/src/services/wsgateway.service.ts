...
export class WSGatewayService {
	...
	tryReconnect = () => {
		if (LOG_LEVEL > 2) console.log("try reconnect");
		setTimeout(() => {
			if (LOG_LEVEL > 2) console.log("reconnecting");
			this.webSocket.io.open((err: any) => {
				if (err) {
					this.tryReconnect();
					return;
				}
				this.webSocket = new Socket({
					url: environment.wsGatewayServer,
	...
	constructor(private router: Router, private cookiesService: CookiesService) {
		this.webSocket = new Socket({
			url: environment.wsGatewayServer,
			//options: { reconnection: true , transports: ["websocket"] },
			options: { reconnection: true },
		}, null);
	...
	allDevices(devicesObj) {
		...
		this.events.allDevices.forEach(fn => fn(this.devices));*/
	}
	receiveStatus() {
